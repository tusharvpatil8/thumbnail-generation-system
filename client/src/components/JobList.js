      ...
      {jobs.length === 0 ? (
        ...
        <div className="space-y-4">
          {jobs.map((job) => {
            ...
            const displayOriginalFilename = getFilenameFromPath(job.originalFile)
              ...
              >
                <div className="mb-4 md:mb-0 md:w-2/3">
                  <p className="font-medium text-white text-lg">
                    File: {job.originalName || displayOriginalFilename || "Unknown file"}
                  </p>
                ...
                {job.status === "completed" && (
                  <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 md:w-1/3 justify-end">
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          if (parent) {
                            ...
                            parent.insertBefore(textPlaceholder, e.target)
                          }
                        }}
                      />
                    ) : (
                    ...
                    <button
                      ...
                      disabled={!displayThumbnailFile}
                      className={`px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors duration-200 text-base font-semibold ${!displayThumbnailFile ? "opacity-50 cursor-not-allowed" : ""}`}
                    >
                      Download
                    </button>
                  </div>
